...
public interface ApplicationContactsRepository3 extends JpaRepository<ApplicationContacts3, Integer> {
	...
	@Transactional
	@Query(value="select a.id,a.applicationid,a.name,"
	+ "a.designation,a.nationality,a.address,"
	+ "a.telephone_number,a.fax_number,a.emailid,"
	+ "mc.Country,ms.statename_inenglish," 
    + "md.districtname_inenglish,a.pincode,a.city from "
    + "application_contacts a inner join m_country mc on a.countryid= mc.id \r\n" + 
    " left join m_states ms on a.stateid = ms.statecode \r\n" + 
    " left join m_districts md on a.districtid= md.districtcode where applicationid "
    + "=:applicationid and contacttype=3", nativeQuery = true)
	List<Object[]> getAllApplicationContactByIdAndAgent(@Param("applicationid") int applicationid);
